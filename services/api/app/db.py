...
from ..migrations import actions, cases, changefeed, evidence, lab, samples, source
...
def migrate(conn: Any) -> None:
    ...
    if isinstance(conn, sqlite3.Connection):
        for statement in source.statements("sqlite") + samples.statements("sqlite") + changefeed.statements("sqlite") + evidence.statements("sqlite") + cases.statements("sqlite") + lab.statements("sqlite") + actions.statements("sqlite"):
            conn.execute(statement)
    else:
        conn.execute(f"CREATE SCHEMA IF NOT EXISTS {SCHEMA}")
        conn.execute(f"SET search_path TO {SCHEMA}")
        for statement in source.statements("postgresql") + samples.statements("postgresql") + changefeed.statements("postgresql") + evidence.statements("postgresql") + cases.statements("postgresql") + lab.statements("postgresql") + actions.statements("postgresql"):
            conn.execute(statement)
    conn.commit()
